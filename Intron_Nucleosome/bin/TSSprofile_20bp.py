...
for almnt in bamfile:
   if almnt.aligned:
      ...
      if almnt.iv.strand == '+':
          almnt.iv.start = almnt.iv.start + fragmentsize - 5
          almnt.iv.end = almnt.iv.start + 10
#          if not almnt.iv.start < 500:
#              coverage[ almnt.iv ] += 1
      else:
          almnt.iv.start = almnt.iv.start + readlen - fragmentsize - 5
          almnt.iv.end = almnt.iv.start + 10
      print almnt.iv
      if not almnt.iv.start < 500:
          coverage[ almnt.iv ] += 1
...
pyplot.savefig('example01_chr1_10bp_bothend.pdf')
